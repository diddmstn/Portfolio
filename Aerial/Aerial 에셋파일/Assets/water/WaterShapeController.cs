...
public class WaterShapeController : MonoBehaviour
{
    ...
    //    Debug.Log("splash");
    //    for (a =0; a < springs.Count;a++) 
    //    {
    //        if(a/2==0)
    //        {
    //            springs[a].velocity += speed;
    //            Debug.Log(a);
    //        }
    //    }
    //}
    IEnumerator Wave(float speed)
    {
        int a = 0;
        for (a = 0; a < springs.Count; a++)
        {
        ...
        WaveStart = false;
        //yield return StartCoroutine(Wave(WaveSpeed));
    }

    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (this.gameObject.CompareTag("Oil"))
        {
            if(collision.CompareTag("Player")||collision.CompareTag("Box"))
            {
                BuoyancyEffector2D buoyancy = GetComponent<BuoyancyEffector2D>();
